...
class LiveAdvisor:
    ...
    def cmd_advice(self, args: str = ""):
        ...
        if not self.board:
            # PREFLOP
            decision = self.preflop.get_decision(
                self.hole_cards, 
                self.position,
                self.facing,
                self.raiser_pos
            ...
            if decision.sizing:
                print(f" to {decision.sizing}bb")
            else:
                print()
                
            if decision.frequency < 1.0:
                print(f"  Frequency: {decision.frequency*100:.0f}%")
            print(f"  💡 {decision.reasoning}")
            
        else:
            # POSTFLOP
            cat, result = self.postflop.analyze_hand_strength(self.hole_cards, self.board)
            print(f"  Hand: {result.description} ({cat.value})")
            
            decision = self.postflop.get_cbet_decision(
                self.hole_cards, self.board, 
                pot_size=self.pot if self.pot > 0 else 10,
                in_position=(self.position in [Position.BTN, Position.CO])
            )
            ...
            if 'BET' in action or action == 'RAISE':
                ...
                if decision.sizing_fraction:
                    print(f" ({decision.sizing_fraction*100:.0f}% pot)")
                else:
                    print()
  ...
  a             Get advice
  a 3b          Advice vs 3-bet
  a vs CO       Advice vs position
