...
try:
    ...
    from src.agents.poker.ai.dynamic_range import DynamicRangeEngine, TableDynamic
...
class PokerAgent:
    ...
    def get_postflop_advice(self, in_position: bool = True) -> Dict:
        ...
        if self.hand_state.bet_to_call > 0:
            decision = self.postflop_engine.get_facing_bet_decision(
                self.hand_state.hole_cards,
                self.hand_state.board,
            ...
            decision = self.postflop_engine.get_cbet_decision(
                self.hand_state.hole_cards,
                self.hand_state.board,
                self.hand_state.pot_size,
                in_position
        ...
        action_style = "value" if decision.action in [PostflopAction.BET_MEDIUM, PostflopAction.BET_LARGE, PostflopAction.RAISE] else "normal"
        if decision.action == PostflopAction.FOLD:
            action_style = "fold"

        self.speak(f"\n→ {decision.action.value.upper()}", action_style)
