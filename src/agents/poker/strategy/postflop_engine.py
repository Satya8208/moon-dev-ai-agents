...
class PostflopEngine:
    ...
    def analyze_hand_strength(self, hole_cards: List[Card], board: List[Card]) -> Tuple[HandCategory, HandResult]:
        ...
        if result.rank == HandRank.ONE_PAIR:
            ...
            if board_ranks:
                if pair_rank >= board_ranks[0]:  # Top pair or overpair
        ...
        # Check for draws
        if analysis.draws:
            strong_draws = [d for d in analysis.draws
                          if d.draw_type in (DrawType.FLUSH_DRAW, DrawType.OESD, DrawType.DOUBLE_GUTSHOT)]
            if strong_draws:
    ...
    def get_cbet_decision(self, hole_cards: List[Card], board: List[Card],
        ...
        if hand_cat in (HandCategory.NUTS, HandCategory.VERY_STRONG, HandCategory.STRONG):
            ...
            has_flush_draw = any(d.draw_type == DrawType.FLUSH_DRAW for d in analysis.draws)
            has_oesd = any(d.draw_type == DrawType.OESD for d in analysis.draws)
    ...
    def get_facing_bet_decision(self, hole_cards: List[Card], board: List[Card],
        ...
        if villain_range:
            ...
            if hand_cat == HandCategory.NUTS:
                ...
                # Count outs
                outs = sum(d.outs for d in analysis.draws)
    ...
    def get_river_value_bet(self, hole_cards: List[Card], board: List[Card],
        ...
        # Bluff candidates
        analysis = self.board_analyzer.analyze(board)
        has_missed_draw = hand_cat == HandCategory.TRASH and len(analysis.draws) > 0
