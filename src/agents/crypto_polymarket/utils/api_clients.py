...
Wrappers for MoonDev API and Polymarket APIs.
...
class MoonDevAPIClient:
    """
    Client for MoonDev API endpoints.
    ...
    def __init__(self, config: CryptoPolymarketConfig):
        self.config = config
        self.base_url = config.moondev_api_url
        self.api_key = config.moondev_api_key
    ...
    def get_liquidation_data(self, symbol: Optional[str] = None, limit: Optional[int] = None) -> pd.DataFrame:
        """
        Fetch liquidation data from MoonDev API.
