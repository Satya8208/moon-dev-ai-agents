"""
Moon Dev's Scalping Strategy Generator Agent v6.4 - DUPLICATE FIX
...
SINGLE_MODEL_CONFIG = {"type": "deepseek", "name": "deepseek-chat"}
...
SCALPING_GENERATION_PROMPT = """You are Moon Dev's Scalping Strategy Generator.
...
def setup_files():
    ...
    if not IDEAS_TXT.exists():
        ...
        with open(IDEAS_TXT, "w") as f:
            f.write("# Moon Dev's Scalping Strategy Ideas\n")
...
def generate_with_single_model(model_config, prompt):
    ...
    try:
        ...
        system_role = """You are Moon Dev's Scalping Strategy Generator. Your ONLY job is to output a complete trading strategy in this EXACT format:
...
def select_mode_interactive():
    ...
    cprint("\n" + "=" * 60, "magenta")
    cprint(" MOON DEV'S SCALPING AGENT v6.4 - ALPHA GENERATION ", "white", "on_magenta")
    cprint("=" * 60, "magenta")
